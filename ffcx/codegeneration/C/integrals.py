...
def generator(ir: IntegralIR, options):
    ...
    if ir.integral_type in ufl.custom_integral_types:
      ...
      code["custom_tabulate_tensor_complex64"] = "NULL"
      code["custom_tabulate_tensor_complex128"] = "NULL"
      np_scalar_type = np.dtype(options["scalar_type"]).name
      code[f"custom_tabulate_tensor_{np_scalar_type}"] = f"custom_tabulate_tensor_{factory_name}"

      implementation = ufcx_integrals.custom_factory.format(
          ...
          custom_tabulate_tensor_complex128=code["custom_tabulate_tensor_complex128"],
      )
    else:
